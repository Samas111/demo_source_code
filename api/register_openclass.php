...
// UPDATED: Check for is_verified instead of just empty email
if ($isVerified == 0) {
    echo json_encode(['status' => 'error', 'code' => 'missing_email']);
    exit;
}

// Fallback check in case email is null but verified is somehow 1
if (empty($userEmail)) {
    echo json_encode(['status' => 'error', 'code' => 'missing_email']);
    exit;
}
...
/* =========================================
   OPENCLASS REGISTRATION NOTIFICATION
========================================= */
// Updated to fetch the creator's public_id AND their custom auto_message
...
if (!empty($creator_public_id) && $creator_public_id !== $public_id) {
    // 1. Push standard notification to the instructor
    ...
    // 2. Send the Automatic DM from instructor to student
    if (!empty($autoMessage)) {
        $dm = $mysqli->prepare("
            INSERT INTO dms (sender_public_id, receiver_public_id, message, is_read) 
            VALUES (?, ?, ?, 0)
        ");
        // Sender is creator, Receiver is the student who just registered ($public_id)
