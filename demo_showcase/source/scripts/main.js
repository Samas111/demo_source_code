...
let currentIndex = items.findIndex(i => i.dataset.page === "dashboard");
...
function loadPageInto(pageEl, showLoading) {
    ...
    fetch(`source/content/${name}.php?ts=${Date.now()}`)
        .then(res => res.text())
        .then(html => {
...
function loadProfileIntoCurrentPage(publicId) {
    ...
    fetch(`source/content/view-profile.php?public_id=${encodeURIComponent(publicId)}`)
        .then(r => r.text())
        .then(html => {
