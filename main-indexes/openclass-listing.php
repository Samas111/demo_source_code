...
$stmt = $mysqli->prepare("
    SELECT
        ...
        cu.public_id AS collab_public_id,
        cu.username AS collab_username,
        cup.pfp_path AS collab_pfp
    ...
    LEFT JOIN users cu 
        ON cu.public_id = o.collab_id 
        AND o.collab_id IS NOT NULL 
        AND o.collab_id != 'none'

    LEFT JOIN user_profile cup 
        ON cup.public_id = cu.public_id
...
/* --------------------------------
   AUTH USER + VIEW LOGGING
-------------------------------- */
$viewer_public_id  = $GLOBALS['AUTH_USER']['public_id'];
$viewer_is_creator = $GLOBALS['AUTH_USER']['is_creator'];
$viewer_username   = $GLOBALS['AUTH_USER']['username'];

$backUrl = ($viewer_is_creator === 0) ? 'app-dancer.php?tab=feed.php' : 'app.php?tab=feed';

/* -------- VIEW LOGIC (20 MIN WINDOW) -------- */
...
<head>
    ...
    <link rel="stylesheet" href="/main-indexes/main-css/openclass.css?v=<?= APP_VERSION ?>">
    <link rel="stylesheet" href="/source/styles/main-app/openclass.css?v=<?= APP_VERSION ?>">
...
<div class="main-content">
    <div class="workshop-card">
        ...
        <div class="workshop-tags">
            ...
            <?php if($led_by != "NONE"){ ?>
                <div class="tag" >
                <?php echo htmlspecialchars($led_by);?>
                </div>
            <?php } ?>
        </div>
        ...
        if ($last24h >= 2): ?>
            <div class="momentum">
                🔥 <?= $last24h ?> registrací za posledních 24 hodin
            </div>
        <?php endif; ?>
    </div>
    ...
    <!-- CREATOR CARD – STATIC FOR NOW -->
        ...
        <?php if (!empty($openclass['collab_public_id'])): ?>
            <div class="creator-card">
                <div class="creator-left" onclick="window.location.href='view-profile.php?public_id=<?= htmlspecialchars($openclass['collab_public_id'], ENT_QUOTES, 'UTF-8') ?>'">
                    <img style="border-radius: 50%;" src="<?= !empty($openclass['collab_pfp']) ? htmlspecialchars($openclass['collab_pfp']) : 'source/assets/default.png' ?>" class="collab-avatar">
                    <div class="creator-info">
                            <div class="creator-top">
                                <span class="creator-name" onclick="window.location.href='view-profile.php?public_id=<?= htmlspecialchars($openclass['creator_public_id'], ENT_QUOTES, 'UTF-8') ?>'">
                                    <?= htmlspecialchars($openclass['collab_username']) ?>
                                </span>
                                <div class="trusted-badge">
                                    <img src="/source/assets/verify_badge.png" alt="">
                                </div>
...
<script>
    ...
    if (btn && !btn.disabled) {
        btn.addEventListener('click', async () => {
            ...
            if (!isRegistered) {
        const res = await fetch('/api/register_openclass.php', {
            method: 'POST',
        ...
        if (data.status === 'error') {
            window.location.href =
                `/main-indexes/openclass-listing.php?id=<?php echo htmlspecialchars($openclassId); ?>&error=${encodeURIComponent(data.code)}`;
            return;
        }

        window.location.href =
            `/main-indexes/openclass-listing.php?id=<?php echo htmlspecialchars($openclassId); ?>&success=registered`;
        return;
    }
...
confirmBtn.onclick = async () => {
    const res = await fetch('/api/unregister_openclass.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: 'openclass_id=<?php echo htmlspecialchars($openclassId); ?>'
    });
