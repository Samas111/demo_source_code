...
require __DIR__ . '/secure/web-logic.php';
...
<head>
    ...
    <link rel="stylesheet" href="main-indexes/main-css/openclass-web.css">
    <link rel="stylesheet" href="main-indexes/main-css/register.css">
...
<div class="app-shell">
    ...
    <div class="bum">
        <div class="guest-register">
            ...
            <p class="login-hint">
                Zrušení registrace je možné v aplikaci
            </p><br>
        </div>
...
<div id="statusPopup" class="popup-overlay" style="display:none;">
    <div class="popup-box" id="popupBox">
        <div class="popup-icon" id="statusIcon">✓</div>
        <h3 id="statusTitle">Ještě jeden krok</h3>
        <p id="statusDesc">
            Poslali jsme ti potvrzovací email. Otevři ho a potvrď registraci – bez toho ti místo nebude rezervováno.
            <br><br>
            Nevidíš ho? Zkontroluj spam nebo hromadné.
        </p>
        <button id="statusClose" class="popup-btn">Rozumím</button>
    </div>
</div>
...
function showStatus(title, desc, isError = false) {
    // Reset classes
    popupBox.className = 'popup-box ' + (isError ? 'error' : 'success');
    statusIcon.innerText = isError ? '✕' : '!';
    statusIcon.style.background = isError ? '#FF5143' : '';
...
form.addEventListener('submit', async (e) => {
    ...
    const userEmail = document.getElementById('guest-email').value;
    
    btn.disabled = true;
    btn.innerText = 'Registruji...';

    try {
        const response = await fetch('/api/register_guest.php', {
            method: 'POST',
        ...
        if (!response.ok || !data || data.success === false) {
            ...
            showStatus('Chyba registrace', errorMsg, true);
            return;
        }
        if (data.success === true) {
            const title = data.resend ? 'Email odeslán znovu' : 'Ještě jeden krok';
            const message = data.resend 
                ? `Původní odkaz byl zneplatněn. Poslali jsme ti nový na ${userEmail}.` 
                : `Poslali jsme ti potvrzovací email na ${userEmail}. Otevři ho a potvrď registraci.`;

            showStatus(title, message + '\n\nNevidíš ho? Zkontroluj spam.');
            
            if (!data.resend) form.reset();
        }
    } catch (err) {
        showStatus('Chyba spojení', 'Nepodařilo se spojit se serverem.', true);
    } finally {
        btn.disabled = false;
        btn.innerText = originalText;
    }
});
statusClose.onclick = () => { statusPopup.style.display = 'none'; };
</script>
</div>
</body>
</html>
