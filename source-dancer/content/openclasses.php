...
if (!empty($public_id)) {
    ...
    if (count($favoriteOpenclasses) < 2) {

        $missing = 2 - count($favoriteOpenclasses);
        ...
        $sql = "
            ...
            LEFT JOIN (
                SELECT 
                    openclass_id,
                    COUNT(DISTINCT viewer_public_id) AS total_views
                FROM openclass_views
                GROUP BY openclass_id
            ) v ON v.openclass_id = oc.openclass_id
        ...
        if (!empty($excludedOpenclassIds)) {
            $types = str_repeat('s', count($excludedOpenclassIds));
            $stmt->bind_param($types, ...$excludedOpenclassIds);
        }
...
if ($registrationsToday > 0) {
    $dopamineText = sprintf($T['dopamine_registrations'], $registrationsToday);
...
if ($registrationsToday === 0) {
    ...
    if ($newOpenclassesToday > 0) {
        $dopamineText = sprintf($T['dopamine_openclasses'], $newOpenclassesToday);
    ...
    if ($newOpenclassesToday === 0) {
        ...
        if ($newPostsToday > 0) {
            $dopamineText = sprintf($T['dopamine_posts'], $newPostsToday);
            $dopamineType = 'posts';
        }
    }
}
...
if (empty($dopamineText)) {
    ...
    if (!empty($trendId) && $trendTotal > 0) {
        ...
        $dopamineText = sprintf($T['dopamine_trending'], $safeTitle);
        $dopamineType = 'trending';
    } else {
        $dopamineText = $T['hype_empty'];
        $dopamineType = 'empty';
    }
}
...
function resolveOpenclassBadge(array $oc): ?array
{
    ...
    if ($createdAt && $createdAt >= strtotime('-3 days')) {
        return ['label' => $T['badge_new'], 'class' => 'badge-trending'];
    }

    if ($regs >= 10) {
        return ['label' => $T['badge_popular'], 'class' => 'badge-trending'];
    }
...
?>
    ...
    <div class="container-overflow">
        ...
        <div class="follow-creators">
            <?php foreach ($creators as $c): ?>
                <div class="creator">
                    <div class="pfp" onclick="window.location='/main-indexes/view-profile.php?public_id=<?php echo htmlspecialchars($c['public_id'])?>'">
                        <img src="<?= htmlspecialchars($c['pfp_path']) ?>" alt="pfp" loading="lazy">
                    </div>
                    <div class="name" onclick="window.location='/main-indexes/view-profile.php?public_id=<?php echo htmlspecialchars($c['public_id'])?>'">
                        <?= htmlspecialchars($c['username']) ?>
                    </div>
                    <?php if (!$c['is_followed']): ?>
                        <button class="follow" data-public-id="<?= htmlspecialchars($c['public_id']) ?>"><?= $T['follow'] ?></button>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>

            <div class="creator search">
                <div class="pfp" onclick="window.location.href='main-indexes/creators.php'">
    ...
    <?php if (!empty($openclasses)): ?>
        <div class="soon">
            <h3><?= $T['from_following'] ?></h3>
            <div class="soon-flex">
                <?php for ($i = 0; $i < 2; $i++): ?>
                    <?php if (isset($openclasses[$i])): 
                        $oc = $openclasses[$i];
                    ?>
                        <div class="openclass-soon"
                            onclick="window.location='/main-indexes/openclass-listing.php?id=<?= htmlspecialchars($oc['openclass_id']) ?>'">
                    ...
                    <?php else: ?>
                        <div class="openclass-soon">
                            ...
                            <div class="soon-cover">
                                <img src="uploads/openclasses/default.png" style="opacity:0;" alt="cover">
                            </div>
                            ...
                            <div class="soon-description">
                                ...
                                <div class="author-soon">
                                    <div class="username" style="opacity:0;">-</div>
                                </div>
                            </div>
    ...
    <?php if (!empty($favoriteOpenclasses)): ?>
        <div class="discover-op">
            <h3><?= $T['popular_dancefy'] ?></h3>
            <div class="dis-flex">

                <?php for ($i = 0; $i < 2; $i++): ?>
                    <?php if (isset($favoriteOpenclasses[$i])): 
                        $oc = $favoriteOpenclasses[$i];
                    ?>
                        <div class="open-card"
                            onclick="window.location='/main-indexes/openclass-listing.php?id=<?= htmlspecialchars($oc['openclass_id']) ?>'">
                    ...
                    <?php else: ?>
                        <div class="open-card">
                            ...
                            <div class="thumbnail-wrapper">
                                <img class="thumbnail" src="/uploads/openclasses/default.png" style="opacity:0;">
                            </div>
                            ...
                            <div class="dis-pad">
                                ...
                                <p style="opacity:0;">- - • -</p>
                                <div class="author-dis">
                                    <img src="default.png" style="opacity:0;" alt="pfp">
                                    <div class="username" style="opacity:0;">-</div>
                                </div>
                            </div>
    ...
    <?php if (!empty($soonOpenclasses)): ?>
        <div class="soon">
            <h3><?= $T['coming_soon_section'] ?></h3>
            <div class="soon-flex">

                <?php for ($i = 0; $i < 2; $i++): ?>
                    <?php if (isset($soonOpenclasses[$i])): 
                        $oc = $soonOpenclasses[$i];
                            ...
                            <div class="soon-description">
                                ...
                                <p class="action-soon">
                                    ...
                                    if ($diff <= 0) {
                                        ...
                                        if ($days > 0) {
                                            echo sprintf($T['in_days'], $days);
                                        } elseif ($hours > 0) {
                                            echo sprintf($T['in_hours'], $hours);
                                        } else {
                                            echo $T['closed'];
                                        }
                                    }
                    ...
                    <?php else: ?>
                        <div class="openclass-soon">
                            ...
                            <div class="soon-description">
                                <span style="opacity:0;">-</span>
                                <p class="action-soon" style="opacity:0;">-</p>

                                <div class="author-soon">
                                    <div class="username" style="opacity:0;">
                                        - | -
                                    </div>
                                </div>
                            </div>
    ...
    <?php endif; ?><br><br><br><br><br><br><br><br>
</div>
