...
---------------------------- */
$oc = $mysqli->prepare("
    SELECT title, capacity, price, description, cover_image, is_qr
    FROM openclasses 
    WHERE openclass_id = ? 
    LIMIT 1
");
...
$oc->execute();
$oc->bind_result($oc_title, $oc_capacity, $oc_price, $oc_description, $cover_image, $is_qr);
...
// REVENUE LOGIC: 
// If is_qr is 1: Show only what is actually marked as paid.
// If is_qr is 0: Show potential revenue (total registrations * price).
if ($is_qr == 1) {
    $revenueLabel = "Potvrzené tržby";
    $currentProfit = $paidDancefyCount * $oc_price;
} else {
    $revenueLabel = "Odhadované tržby";
    $currentProfit = $totalRegistrations * $oc_price;
}
...
$capacityPercent = 0;
if ($oc_capacity > 0) {
    // If QR is on, progress reflects only paid. If off, progress reflects all registrations.
    $countForProgress = ($is_qr == 1) ? $paidDancefyCount : $totalRegistrations;
    $capacityPercent = min(100, round(($countForProgress / $oc_capacity) * 100));
}
...
<head>
    ...
    <link rel="stylesheet" href="/source/styles/app-index/attendance.css?v=<?= APP_VERSION ?>">
...
<?php else: ?>
    ...
    <div class="hint">
        <p>
            ☀️ Dnes <?= $todayCount ?> 
            <?= ($todayCount === 1 ? 'registrace' : ($todayCount < 5 ? 'registrace' : 'registrací')) ?> 
            • Plní se to 🔥
        </p>
    </div>

    <?php elseif ($totalRegistrations > 10): ?>
    ...
    <?php else: ?>
        <div class="hint">
            <p>První registrace přišly 🚀 <br> Využij Instagram a naplň lekci rychleji 🔥</p>
        </div>
    <?php endif; ?>
    ...
    <?php if ($oc_capacity > 0): ?>
        <div class="capacity-container">
            <div class="capacity-top">
                <span class="capacity-label"><?= ($is_qr == 1) ? 'Placené registrace' : 'Obsazenost' ?></span>
                <span class="capacity-count">
                    <?php 
                        $displayCount = ($is_qr == 1) ? $paidDancefyCount : $totalRegistrations;
                    ?>
                    <strong><?= $displayCount == 0 ? '—' : $displayCount ?></strong> / <?= $oc_capacity ?>
                </span>
            </div>
...
<div class="registrations">
    ...
    <p style="font-weight: 400;">Dancefy Uživatelé <img src="/source/assets/info.png" class="info-trigger" data-info-key="dancefy_users"></p>
    <?php if (!empty($regs)): ?>
        <div class="dancefy-users">
        <?php foreach ($regs as $user): ?>
            <?php 
                $isUnpaidStyle = ($is_qr == 1 && (int)$user['is_paid'] === 0) ? 'unpaid' : '';
            ?>
            <div class="row-dancefy row-link <?= $isUnpaidStyle ?>" onclick="window.location.href='/main-indexes/view-profile.php?public_id=<?= $user['public_id'] ?>&from=attendance&id=<?= urlencode($openclassId) ?>'">
                
                <img class="pfp" src="<?= htmlspecialchars('/' . ltrim($user['pfp'], '/')) ?>">
                <span>
                <?php
                    $name = trim($user['name'] ?? '');

                    if ($name !== '' && $name !== 'Dancefy Uživatel') {
                ...
                <?php if ($user['is_today']): ?>
                    <span class="badge-today" style="width: max-content !important; text-align: center !important;">Nové Dnes</span>
                <?php endif; ?>
    ...
    <?php if (!empty($webRegs)): ?>
        <p style="font-weight: 400;">Hosté (<?= count($webRegs) ?>) <img src="/source/assets/info.png" class="info-trigger" data-info-key="guests"></p>
        <div class="users">
            <?php foreach ($webRegs as $guest): ?>
                <div class="row">
                    <div class="circle" style="font-weight: 500;"><?= htmlspecialchars($guest['initials']) ?></div>
                    <div class="field">
                        <span style="font-weight: 600;"><?= htmlspecialchars($guest['name']) ?></span>
                        <?php if ($guest['is_today']): ?>
                            <span class="badge-today" style="font-weight: 600; opacity: 1 !important;">Dnes</span>
                        <?php endif; ?>
                        <p style="font-weight: 500;" class="wmail"><?= htmlspecialchars($guest['email']) ?></p>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <?php if ($totalCancelled > 0): ?>
        <p style="font-weight: 400;">Zrušeno <img src="/source/assets/info.png" class="info-trigger" data-info-key="cancelled"></p>
        <div class="users-storno">
            <?php foreach ($cancelled as $user): ?>
                <div class="row-storno">
                    <div style="font-weight: 500;" class="circle"><?= mb_strtoupper(mb_substr($user['username'], 0, 2)) ?></div>
                    <span style="font-weight: 500;"><?= htmlspecialchars($user['username']) ?></span>
...
<div id="shareSheet" class="share-sheet hidden">
    ...
    <div class="share-panel">
        ...
        <div class="share-actions">
            <button id="dancefyShare" style="display: none;">
...
<div id="ocActionSheet" class="oc-sheet hidden">
    ...
    <div class="oc-panel">
        ...
        <button class="oc-btn danger" style="display: none;" onclick="window.location.href='cancel.php?id=<?= urlencode($openclassId) ?>'">Zastavit OpenClass</button>
...
// INFO MODAL LOGIC
const INFO_CONTENT = {
    dancefy_users: { title: 'Dancefy uživatelé', text: 'Tanečníci, kteří se zaregistrovali přes aplikaci. Mají reputaci a portfolio. Můžete jim napsat.' },
    guests: { title: 'Hosté', text: 'Registrovaní uživatelé přes web. Nemají reputaci ani historii. Jsou ověření přes email.' },
    cancelled: { title: 'Zrušené registrace', text: 'Uživatelé, kteří registraci zrušili. Storno ovlivňuje reputaci tanečníka.' }
};
...
async function togglePayment(publicId, container) {
    ...
    // Visual feedback (optional opacity while loading)
    container.style.opacity = '0.5';

    try {
        const response = await fetch('/api/toggle_paid.php', {
            method: 'POST',
