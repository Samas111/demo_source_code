...
$ibanStmt = $mysqli->prepare("SELECT iban FROM user_profile WHERE public_id = ? LIMIT 1");
$ibanStmt->bind_param('s', $public_id);
$ibanStmt->execute();
$ibanStmt->bind_result($iban);
$ibanStmt->fetch();
$ibanStmt->close();
...
/* ----------------------------
    ...
    /* Build message */
    $dopamineMessage = sprintf($T['welcome_back'], $usernameSafe);
    if ($todayRegistrations > 0) {
        $dopamineMessage = sprintf($T['dopamine_regs_creator'], $todayRegistrations);
    } elseif ($todayFollowers > 0) {
        $dopamineMessage = sprintf($T['dopamine_flws_creator'], $todayFollowers);
    } elseif ($todayViews > 0) {
        $dopamineMessage = sprintf($T['dopamine_views_creator'], $todayViews);
    }
    ...
    if ($public_id) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $row['countdown'] = sprintf($T['countdown_format'], $days, $hours, $minutes);
...
foreach ($openclasses as &$oc) {
            ...
            // 2. Fetch Individual Views for this OpenClass (The missing part)
            $ocViewStmt = $mysqli->prepare("
                SELECT COUNT(*) 
                FROM openclass_views 
                WHERE openclass_id = ?
            ");
            $ocViewStmt->bind_param('s', $oc['openclass_id']);
            $ocViewStmt->execute();
            $ocViewStmt->bind_result($individualViews);
            $ocViewStmt->fetch();
            $ocViewStmt->close();

            $oc['attendance'] = (int)$attendance;
            $oc['views'] = (int)$individualViews; // Now this key exists!
...
if (!empty($public_id)) {
    $notifStmt = $mysqli->prepare("
        ...
        LEFT JOIN user_profile up ON up.public_id = n.actor_public_id
        WHERE n.recipient_public_id = ?
        ORDER BY n.created_at DESC
        LIMIT 3
    ");
    $notifStmt->bind_param("s", $public_id);
    $notifStmt->execute();
    $notifResult = $notifStmt->get_result();
    
    while ($row = $notifResult->fetch_assoc()) {
        $row['actor_pfp'] = !empty($row['actor_pfp']) 
            ? ltrim($row['actor_pfp'], '/') 
            : 'source/assets/default.png';
        $topNotifications[] = $row;
    }
    $notifStmt->close();
}
...
function renderBottomLine(int $value): string
{
    global $T;
    if ($value > 0) {
        return '<div class="bottom-line">+' . $value . ' ' . $T['today'] . '</div>';
    }
    ...
    if (!$egoMessage) {
        ...
        while ($row = $result->fetch_assoc()) {
            if ($row['public_id'] === $public_id) {
                $egoMessage = $T['ego_top5_followers'];
                break;
            }
        }
...
<div class="container-overflow">
        ...
        <div class="profile-warning" onclick="window.location='/settings/payments.php'">
            <div class="warning-left">
                ...
                <div class="warning-text">
                    <span class="warning-title"><?= $T['iban_title'] ?></span>
                    <span class="warning-sub"><?= $T['iban_sub'] ?></span>
                </div>
            </div>
            <div class="warning-action"><?= $T['add'] ?></div>
        </div>
    <?php endif; ?>
    <div class="active-workshops">
                        ...
                        <div class="content">
                            ...
                            <div class="workshop-stats">
                                ...
                                <div class="stat cash">
                                    ...
                                    <?= number_format($oc['views'], 0, ',', ' ') ?>
                                </div>
                            </div>
                            <div class="bottom-nav">
                                <div class="buttons-nav">
                                    <button onclick="window.location.href='source/app-index/attendance.php?id=<?= urlencode($oc['openclass_id']) ?>'"><?= $T['registrations'] ?></button>
                                    <!-- <button onclick="window.location.href='#'">Upravit</button> -->
                                </div>
                            </div>
                        </div>
                        <div class="image">
        ...
        <?php else: ?>
            <?php
                $ctaTitle = $T['create_oc'];
                $ctaSubtitle = $T['create_oc_sub'];

                if ($creatorTotalFollowers > 0) {
                    $ctaTitle = sprintf($T['followers_have'], $creatorTotalFollowers);
                    $ctaSubtitle = $T['waiting_next_oc'];
                }
                if ($todayFollowers > 0) {
                    $ctaSubtitle = $T['waiting_next_oc'];
                }
                ...
                <div class="add-openclass-container" onclick="window.location='post/post.php'">
    ...
    <div class="notif-container">
        <div class="notif-header">
            ...
            <a href="source/app-index/notifications.php" class="notif-link"><?= $T['view_all'] ?></a>
        </div>

        <div class="notif-list">
            <?php if (empty($topNotifications)): ?>
                <div class="notif-empty" style="padding: 20px; text-align: center; opacity: 0.5;">
                    <?= $T['no_notifications'] ?>
                </div>
            <?php else: ?>
                <?php foreach ($topNotifications as $n): 
                    ...
                    if (str_contains($titleLower, 'registr')) {
                        $badgeClass = 'plus';
                        $badgeIcon  = '+';
                    } elseif (str_contains($titleLower, 'sledo') || str_contains($titleLower, 'follower')) {
                        $badgeClass = 'follow'; 
                        $badgeIcon  = '👤';
                    } elseif (str_contains($titleLower, 'zpráv') || str_contains($titleLower, 'message')) {
                        $badgeClass = 'msg';
                        $badgeIcon  = '💬';
                    } else {
                        $badgeClass = 'info';
                ...
                ?>
                    <div class="notif-row <?= $n['is_read'] ? '' : 'highlight' ?>">
                        <div class="avatar-wrapper">
                            <img src="<?= htmlspecialchars($n['actor_pfp']) ?>" class="avatar-plain" alt="user">
                            <div class="icon-badge <?= $badgeClass ?>">
                                <?= $badgeIcon ?>
                            </div>
                        </div>
                        <div class="notif-content">
                            <p class="notif-text"><?= htmlspecialchars($n['title']) ?></p>
                            <p class="notif-sub">
                                <?= htmlspecialchars(mb_strimwidth($n['content'], 0, 35, '...', 'UTF-8')) ?>
                            </p>
                        </div>
                    </div>
                <?php endforeach; ?>
    ...
    <div class="creator-level-card">
        ...
        <div class="level-header">
            ...
            <div class="level-boost">
                <span class="boost-value">+<?= (int)$level['boost'] ?>%</span> <?= $T['visibility_boost'] ?>
            </div>
        </div>
        ...
        <div class="level-tasks">
            ...
            <?php if ($level['needs_share_post']): ?>
                <div class="task <?= $hasPost ? 'completed' : 'active' ?>">
                    ...
                    <div class="task-text">
                        <div class="task-title"><?= $T['task_share_post'] ?></div>
                        <div class="task-desc"><?= $T['task_share_post_sub'] ?></div>
                    </div>
                </div>
            <?php endif; ?>
                <div class="task <?= $creatorTotalRegistrations >= $level['registrations_required'] ? 'completed' : 'active' ?>">
                    ...
                    <div class="task-text">
                        ...
                        <div class="task-desc"><?= $T['task_get_regs_sub'] ?></div>
                    </div>
                </div>
                <div class="task <?= $creatorTotalFollowers >= $level['followers_required'] ? 'completed' : 'active' ?>">
                    ...
                    <div class="task-text">
                        ...
                        <div class="task-desc"><?= $T['task_get_followers_sub'] ?></div>
                    </div>
                </div>
            </div>
        </div>
